...
discount_factor = 0.9
R = 200.0
methods = ["ATA-FMDP", "ATA-MaxMin", "FPL-MTD", "EPS-GREEDY", "URS"]
random.seed(10)
...
if __name__ == "__main__":
    ...
    for alpha in alphas:
        ...
        for ln in turn_len:
            ...
            plt.close('all')
            plt.plot(range(timesteps), [val / iter for val in sum_reward_timesteps_maxmin], label="Adaptive-Threat-Aware-MaxMin", color="r")
            ...
            plt.xlabel("Timestep")
            # plt.savefig("./graphs/evolving/ts_stochastic_evolve_mdp_comp_alpha_" + str(alpha) + "_5_fold.png")
            plt.savefig("./graphs/evolving/ts_stochastic_evolve_maxmin_comp_alpha_" + str(alpha) + "_5_fold.png")
            ...
            # plt.close('all')
            # fig, ax = plt.subplots()
            # bars = ax.bar(methods, mean_values, yerr=std_values, capsize=6)
            # bars[0].set_color('red')
            # ax.set_ylabel('Average Defender Reward')
            # ax.set_xlabel('Method')
            # plt.yticks(np.arange(0, 210, 10))
            # plt.tight_layout()
            # plt.savefig("./graphs/evolving/stochastic_evolve_mdp_maxmin_alpha_" + str(alpha) + "_5_fold.png")
        
        # plt.plot(turn_len, maxmin_adapts, label="Adaptive-Threat-Aware-MaxMin", color="b")
        # plt.plot(turn_len, mdp_adapts, label="Adaptive-Threat-Aware-MDP", color="r")
