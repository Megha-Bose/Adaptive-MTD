...
if __name__ == "__main__":
    db_switch = 0
    to_mysql_switch_count = 0

    lang_switch = 0
    to_php_switch_count = 0
    ...
    for alpha in alphas:
        ...
        for it in range(iter):
            ...
            for num in range(timesteps):
                ...
                if current_mdp_state[0] != current_mdp_action[0]:
                    lang_switch +=1
                    if current_mdp_action[0] == 1:
                        to_php_switch_count += 1
                if current_mdp_state[2] != current_mdp_action[2]:
                    db_switch +=1
                    if current_mdp_action[2] == 1:
                        to_mysql_switch_count += 1
        ...
        plt.savefig("./graphs/non-evolving/ts_stochastic_non_evolve_DH_alpha_" + str(alpha) + ".png")
        ...
        plt.yticks(np.arange(0, 210, 10))
        plt.tight_layout()
        plt.savefig("./graphs/non-evolving/stochastic_non_evolve_DH_alpha_" + str(alpha) + ".png")

    print("Total switches made in DB: ", db_switch)
    print("Total switches made to MySQL DB: ", to_mysql_switch_count)
    print("Total switches made in Language: ", lang_switch)
    print("Total switches made to PHP: ", to_php_switch_count)