from get_attacker_exploits import *
from itertools import product
import gurobipy as gp
from gurobipy import GRB

def loader():
    parent_path = './data/nvd_based/web_app/'
    ...
    # Load attacker information
    sys_att_type_exp = {}
    # Read the attacker file
    with open(parent_path + 'attackers.txt', 'r') as file:
        lines = file.readlines()
    # Split the lines into attacker types and their expertise values
    attacker_types = lines[0].strip().split(',')
    # Create the attacker type expertise dict
    for i, att_type in enumerate(attacker_types):
        sys_att_type_exp[att_type] = {}
        techs = lines[i + 3].strip().split(',')
        for tech in techs:
            tech_name, tech_exp = tech.split('-')
            sys_att_type_exp[att_type][tech_name] = int(tech_exp)
    exploits = get_exploit_sets(sys_techs, sys_att_type_exp)
    ...
    return state_factors, action_factors, attacker_types, exploits, switching_costs
...
state_factors, action_factors, attacker_types, exploits, switching_costs = loader()
print(exploits)