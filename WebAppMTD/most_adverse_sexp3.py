...
if __name__ == "__main__":
    ...
    for alpha in alphas:
        ...
        for it in range(iter):
            ...
            attacker_estimated_sexp3_defender_strategy = {}
            for state in states:
                attacker_estimated_sexp3_defender_strategy[state] = {}
                for action in actions:
                    attacker_estimated_sexp3_defender_strategy[state][action] = 0.0
            ...
            for num in range(timesteps):
                ...
                true_prob_att_type = get_att_type_probs(current_sexp3_state, attacker_estimated_sexp3_defender_strategy, true_prob_att_success, attack_loss, attacker_types)
                types = list(true_prob_att_type.keys())
                type_probabilities = list(true_prob_att_type.values())
                current_sexp3_att_type = random.choices(types, type_probabilities)[0]
                ...
                attacker_estimated_sexp3_defender_strategy[current_sexp3_state][current_sexp3_action]+=1
                if current_sexp3_state != current_sexp3_action:
                    num_switches["S-EXP3"] += 1.0
        ...
        plt.tight_layout()
        plt.savefig("./graphs/ts_most_adverse_evolve_sexp3_alpha_" + str(alpha) + ".png")
        ...
        print("S-EXP3: ", np.mean(cumulative_rewards_sexp3), np.std(cumulative_rewards_sexp3), num_attack_success["S-EXP3"]/iter, num_switches["S-EXP3"]/iter)
        ...
        plt.tight_layout()
        plt.savefig("./graphs/most_adverse_evolve_sexp3_alpha_" + str(alpha) + ".png")